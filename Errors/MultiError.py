...
class MultiError:
    ...
    def measure1(self, config, info, train_data, test_data):
        # Post Initializing
            ...
            with tf.Session() as sess:
                ...
                if self._loadCheckpointSaver(config, saver, sess):
                    for training_indices, training_ratings, \
                        ...
                        iterate_multi_test_mini_batch(self._nCriteria,
                                                      self._train,
                                                      self._test,
                                                      self._batch_size):
    ...
    def measure2(self, config, info, train_data, test_data):
        # Post Initializing
        self._post_init(config, info, train_data, test_data)
        ...
        with tf.Graph().as_default():
            ...
            test_indices = tf.placeholder(tf.int32, name='output_indices')
            test_ratings = tf.placeholder(tf.float32, name='output_ratings')
            test_shape = [self._batch_size, self._input_nuerons]

            test_Y = tf.sparse_to_dense(test_indices, test_shape, test_ratings)

            Y_out = model.inference2(self._input_nuerons,
                                    self._output_nuerons,
                                    X)
            ...
            with tf.Session() as sess:
                ...
                if self._loadCheckpointSaver(config, saver, sess):
                    print("hello")
                    input_dataset = iterate_multi_test_mini_batch_2(self._nCriteria,
                                                      self._train,
                                                      self._test,
                                                      self._batch_size,
                                                      self._input_nuerons)
                    print("Number of Batches: {}".foramt(len(input_dataset)))
                    tmp_counter = 1
                    for input_data in input_dataset:
                        print("Current Running Batch {}/{}".format(tmp_counter, len(input_dataset)))
                        tmp_counter += 1
                        curr_mae, curr_rms = sess.run([mae_op, rms_op],
                                                      {X: input_data[0],
                                                       test_indices: input_data[1],
                                                       test_ratings: input_data[2]})
